...
def create_dataset_yaml(dataset_root, output_dir):
    """
    Create YOLO format yaml configuration file based on existing dataset structure
    ...
    dataset_info = {
        'path': dataset_root,  # Use dataset root directory
        'train': 'train/images',  # Training set image directory
        'val': 'valid/images',  # Validation set image directory (note: this is "valid" not "val")
        'test': 'test/images',  # Test set image directory
        'names': class_names,  # Class names
        'nc': len(alphabet)  # Number of classes
    }
...
def fine_tune_yolo(dataset_yaml, epochs=100, batch_size=16, image_size=640, device="0", save_format="pth"):
    """
    Fine-tune YOLOv8 model and save in specified format, while recording the training process
    ...
    - save_format: Save format ("pt" or "pth")
    """
    # Load pre-trained YOLOv8 model
    model = YOLO("yolov8n.pt")  # Use YOLOv8 nano version
    ...
    results = model.train(
        ...
        lrf=0.01,  # Final learning rate coefficient
        ...
        degrees=15.0,  # Rotation angle (±degrees)
        translate=0.1,  # Translation (±fraction of image width/height)
        scale=0.5,  # Scaling (±percentage)
        ...
        mixup=0.3,  # Mixup augmentation probability - changed from 0 to 0.3, enabling MixUp augmentation
        copy_paste=0.0,  # Copy-paste probability
        # Use YOLOv8 supported random erasing parameters (only use erasing parameter)
        erasing=0.4,  # Random erasing probability
        cache=True,  # Cache images to speed up training
        exist_ok=True,  # Allow overwriting existing experiment directory
        workers=8,  # Number of data loader worker threads
...
def test_model(model, test_images_dir, conf_threshold=0.25):
    ...
    results = model.predict(
        ...
        save_crop=True,  # Save cropped prediction results
        verbose=True,  # Verbose output
    )
...
def main():
    ...
    model, results = fine_tune_yolo(
        ...
        device="0",  # Use first GPU, use "cpu" for CPU
        save_format="pth"  # Save as .pth format
    )
